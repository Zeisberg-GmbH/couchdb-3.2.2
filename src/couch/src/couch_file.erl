...
%% couch_file:open, which couch_file uses to wrap the data encryption
%% key using AES-GCM with a random IV. couch_file stores the IV and
%% the wrapped key in a special header block, always block 0, in the
%% first 4 KiB of the file.  All data, including all headers except
%% the encryption header at block 0, are encrypted using AES in
%% Counter Mode, where the counter is calculated as IV plus the file
%% offset, in AES block increments.
